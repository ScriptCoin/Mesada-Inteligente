...
function logRequest(requestBody, path) {
  console.log("URL : " + config.visa.visaUrl + path);
...
function getXPayToken(resourcePath, queryParams, postBody) {
  ...
  var sharedSecret = config.visa.sharedSecret;
...
VisaAPIClient.prototype.doMutualAuthRequest = function(path, requestBody, methodType, headers, callback) {
  ...
  var userId = config.visa.userId;
  var password = config.visa.password;
  var keyFile = config.visa.key;
  var certificateFile = config.visa.cert;
  ...
  headers['ex-correlation-id'] = randomstring.generate({
      length: 12,
      charset: 'alphanumeric'
    }) + '_SC'
  request({
    uri: config.visa.visaUrl + path,
...
VisaAPIClient.prototype.doXPayRequest = function(baseUri, resourcePath, queryParams, requestBody, methodType, headers, callback) {
  ...
  request({
    uri: config.visa.visaUrl + baseUri + resourcePath + '?' + queryParams,
    method: methodType,
    headers: headers,
    body: requestBody
  }, function(error, response, body) {
